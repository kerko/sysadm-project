...
The next last step takes the longest, go into your vagrant directory and just do: \mint{bash}|vagrant up| Vagrant now takes care of everything, imports the basebox,including downloading it from Amazon S3, creating the \gls{VM} and running puppet to provision everything. The first run takes something between 20-40 minutes depending on your internet connection since puppet download the docker images the first time its run and also all the software.
